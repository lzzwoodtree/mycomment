define(["./has!dom-addeventlistener?:./aspect", "./_base/kernel", "./sniff"], function(aspect, dojo, has){
	...
	// on主要通过element自身的addEventListener方法来实现
	// emit方法：若target没有addEventListener方法则通过调用target[method]方法使用，
	// 否则通过模拟事件来触发

	// 由on方法调用parse，再由parse调用addListener
	...
	// 继承了Evented类则调用Evented中的on方法(本质是调用aspect.after方法);
	// 否则由on模块解析并调用自身addListener方法
	...
	on.parse = function(target, type, listener, addListener, dontFix, matchesTarget){
		if(type.call){
			...
			return type.call(matchesTarget, target, listener); // 使用on.selector方法时从此处返回
		}
